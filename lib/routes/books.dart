...
main() async {
  // The URL for the demo bibliotech server
...
Future<Stream<Book>> getAllBooks() async {
  ...
  // Get the list of all books from the server
  var uri = Uri.parse(config.hostname + "/books");
  var request = new http.Request('get', uri);

  var response = await client.send(request);

  // Decode the response as UTF-8 JSON data and turn it into Book items
...
Future<Stream<Book>> searchAllBooks(String searchTerm) async {
  var client = new http.Client();
  // Get the list of all books matching a search term
...
Future<Stream<Book>> getAllMyBooks() async {
  var client = new http.Client();
  // Get the list of all books checked out by the user who is logged in
...
Future<Stream<Book>> getBooksWithDewey(int range) async {
  var client = new http.Client();
  // Get all books within the specified range (where range = X00, e.g. 100, 900, etc.)
  ...
  return response.stream
    ...
    .asyncMap((jsonBook) async {
      // Get the book info for the specified book with the ISBN from the server
...
Future<List> getFromTwitter(Book book) async {
  // Set up the TwitterApi with the OAuth data and search for all results related to the specified book
...
Future<Book> getBook(String isbn) async {
  ...
  final json = JSON.decode(response.body);
  // Assuming the book was found, convert it to a Book object
  if (json == null) {
    return null;
  } else {
    print(response.body);
